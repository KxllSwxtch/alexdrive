...
class TestLifespan:
    ...
    async def test_lifespan_continues_on_prewarm_failure(self):
        ...
        with patch("app.main.get_filter_data", new_callable=AsyncMock, side_effect=RuntimeError("fail")), \
             patch("app.main.get_car_listings", new_callable=AsyncMock, side_effect=RuntimeError("fail")):
