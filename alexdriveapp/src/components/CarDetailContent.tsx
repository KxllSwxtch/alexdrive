...
export async function CarDetailContent({ id }: { id: string }) {
  ...
  // Parse price for credit calculator and display: "3,240만원" → 32400000 KRW
  const manWon = parseManWonFromString(car.price);
  const carPriceKrw = manWon ? manWonToKrw(manWon) : 0;
  const displayPrice = manWon ? formatPriceKrw(manWon) : car.price;
  const translatedName = translateSmartly(car.name);
  const shareTitle = `${translatedName} ${car.year}${displayPrice ? ` - ${displayPrice}` : ""}`;

  const specs = [
    { label: "Год выпуска", value: car.year },
    { label: "Год модели", value: car.modelYear },
    { label: "Дата регистрации", value: car.registrationDate },
    { label: "Пробег", value: car.mileage },
    { label: "Топливо", value: translateSmartly(car.fuel) },
    { label: "КПП", value: translateSmartly(car.transmission) },
    { label: "Цвет", value: translateSmartly(car.color) },
    { label: "Объём двигателя", value: car.engineCapacity },
    { label: "Гос. номер", value: car.carNumber },
    { label: "Местоположение", value: car.location },
  ].filter((s) => s.value);
  ...
  return (
    <>
      ...
      <div className="min-w-0 space-y-6">
        {/* Gallery */}
        <ImageGallery images={car.images} alt={car.name} blurDataUrl={car.blurDataUrl} />
        ...
        {car.inspectionUrl && (
          <a
            ...
            className="flex items-center justify-center gap-2 rounded-xl border border-border bg-bg-surface p-4 text-sm font-medium text-gold transition-colors hover:bg-gold/5 hover:border-gold/30"
          >
        ...
        {car.options.length > 0 && (
          <CarOptions options={car.options.map(group => ({
            group: translateSmartly(group.group),
